...
from upper_bound import filter_rows, tokenize, MONTHS, MONTHS_MAP, IDEOLOGIES, COMPS, SPLITS, N_SENTS, SPLITS, scorer, read_data, TOPIC_MAP
import h5py
...
def writer(queue, topic):
    path = "{}/cbc_temporal_{}.csv".format(PROJ_ROOT, topic)
    logger.info("writing output to %s"%path)
    fout = open(path, "w")
    fout.write("seed,topic,comp,ideology,method,k,val,test,summ1,summ2,hyps\n")
...
def run(df1, idxs_tr, idxs_te, y_tr, y_te, steps, mk, seed, topic, ideology, label1, label2, callback = lambda x: x):
    try:
        ...
        #### mistake, to fix
        summ_idxs = idxs_tr[S]
        assert set(df1.month.iloc[summ_idxs[:mk]]) == {label1} and set(df1.month.iloc[summ_idxs[mk:]]) == {label2}# ( set(df1.ideology.iloc[summ_idxs[:mk]]), label1, set(df1.ideology.iloc[summ_idxs[mk:]]), label2 )
        ...
        msg = "{},{},{}_{},{},{},{},{:.4g},{:.4g},{},{},{}".format(
            seed, TOPIC_MAP[topic], MONTHS_MAP[label1], MONTHS_MAP[label2], ideology, name, 
        ...
        msg = "{},{},{}_{},{},{},{}".format(
            seed, TOPIC_MAP[topic], MONTHS_MAP[label1], MONTHS_MAP[label2], ideology, name, mk
        )
...
def main():
    ...
    for ideology in IDEOLOGIES:
        df1 = df.query('ideology == @ideology').reset_index(drop=True)
        logger.info( "topic:{}, {}, #articles:{}, {}".format(topic, ideology, len(df1), Counter(df1["month"]) ) )


        for (label1, label2) in COMPS:
            logger.debug("comparing %s-%s, %s"%(MONTHS_MAP[label1], MONTHS_MAP[label2], ideology))
            ...
            for seed in SPLITS:
                ...
                for steps, mk in product(STEPS, mks):    
                    job = pool.apply_async(run, (df2, idxs_tr, idxs_te, y_tr, y_te, steps, mk, seed, topic, ideology, label1, label2, queue.put))
                    jobs.append(job)
