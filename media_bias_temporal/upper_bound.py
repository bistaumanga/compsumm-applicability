...
# assert len(MONTHS_MAP) == 9

IDEOLOGIES = ["ER", "R", "RC", "C", "LC", "L"]
COMPS = [(a, b) for (a, b) in product(MONTHS, MONTHS) if a < b]
...
def filter_rows(df, label1, label2):
    idx1 = np.where(df.month == label1 )[0]
    idx2 = np.where(df.month == label2 )[0]
...
def writer(queue, topic, feats, method):
    fout = open("%s/%s_temporal_%s_%s_nonorm.csv"%(PROJ_ROOT, method, topic, feats), "w")
    fout.write("topic,comp,seed,ideology,num_tr1,num_tr2,num_te1,num_te2,val,test,num_feats,time\n")
...
def read_data(topic, extract_tokens = True):
    ...
    df = df.query("ideology in @IDEOLOGIES").reset_index()
    df["sents"] = [ s[:N_SENTS] for s in df["sents"] ]
    df["month"] = df["date"].apply( lambda row: row.strftime('%Y-%m') )
...
def main():
    ...
    for ideology in IDEOLOGIES:
        ...
        for (label1, label2) in COMPS:
            ...
            logger.info("{}-{}: X,shape = {} in {} secs".format(
                MONTHS_MAP[label1], MONTHS_MAP[label2], X.shape, time.time() - start ))
            ...
            for seed in SPLITS:
                ...
                msg = "{},{}_{},{},{},{},{},{},{}".format(
                    TOPIC_MAP[topic], MONTHS_MAP[label1], MONTHS_MAP[label2], seed, ideology,
