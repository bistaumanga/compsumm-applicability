...
for (topic in topics){
  ...
  cbc_bow <- read.csv(sprintf("%s/res/cbc_temporal_%s.csv", script.dir, topic)) %>%
...
ggsave(sprintf("%s/plots/%s/paired_svm.%s", script.dir, "both", save_as),
       plot = gg.paired,
...
for (t in unique(summ.ideologies$Topic)){
  ...
  ggsave(sprintf("%s/plots/%s/error2d_%s.%s", script.dir, "both", t, save_as),
         plot = error.2d,
...
write.csv(summ.ideologies.topics %>% 
            ...
            mutate(by = "Month"), 
          sprintf("%s/summ_temporal.csv", script.dir), 
          row.names = F, quote = F)
...
ggsave(sprintf("%s/plots/%s/balloon_temporal.%s", script.dir, "both", save_as),
       plot = ggb,
...
ggsave(sprintf("%s/plots/%s/line.%s", script.dir, "both", save_as),
       plot = gg.line,
...
ggsave(sprintf("%s/plots/%s/ideology_temporal.%s", script.dir, "both", save_as),
       plot = gg.tmp,
...
ggsave(sprintf("%s/plots/%s/ideology_temporal2.%s", script.dir, "both", save_as),
       plot = gg.tmp2,
...
#   theme_bw()
# 
# ggsave(sprintf("%s/plots/%s/ideology_temporal2box.%s", script.dir, "both", save_as),
#        plot = gg.tmp2.box,
...
#   theme_bw()
# 
# ggsave(sprintf("%s/plots/%s/ideology_temporal3.%s", script.dir, "both", save_as),
#        plot = gg.jsd.tmp,
