...
for (topic in topics){
  ...
  cbc_random <- read.csv(sprintf("%s/random/cbc_ideology_%s_random.csv", script.dir, topic)) %>%
    mutate(feats = case_when(method =="tok_idf-bow_cos_random" ~ "BoW", 
                             method == "emb_cos_random" ~ "SBERT", TRUE ~ "NA"))
  
  cbc_mmd <- read.csv(sprintf("%s/res/cbc_ideology_%s.csv", script.dir, topic)) %>%
...
cbc_random <- cbc_random %>% select(-c(val, summ1, summ2, hyps)) %>%
...
cbc_mmd <- cbc_mmd %>% select(-c(val, summ1, summ2, hyps)) %>%
  group_by(feats, comp, topic, k, month) %>% ## some weird bug
...
write.csv(cbc %>% 
            ...
            mutate(by = "Ideology"), 
          sprintf("%s/joined_ideology.csv", script.dir), 
          row.names = F, quote = F)
...
# #
# # ggsave(sprintf("%s/plots/%s_random/svm.%s", script.dir, Features, save_as),
# #        plot = gg.bar,
...
# 
# ggsave(sprintf("%s/plots/%s_random/paired_svm.%s", script.dir, Features, save_as),
#        plot = gg.paired,
...
# 
#   ggsave(sprintf("%s/plots/%s_random/error2d_%s.%s", script.dir, Features, t, save_as),
#          plot = error.2d,
...
#             mutate_if(is.numeric, round, digits = 4) %>%
#             mutate(by = "Ideology"),
#           sprintf("%s/summ_ideology_random.csv", script.dir),
#           row.names = F, quote = F)
# # # ######## balloon plot ##########
# 
# #
...
# 
# ggsave(sprintf("%s/plots/%s_random/balloon_ideology_pres.%s", script.dir, "both", save_as),
#        plot = ggb1,
...
#   theme_bw() + theme(legend.position = "bottom")
# 
# ggsave(sprintf("%s/plots/%s_random/balloon_ideology.%s", script.dir, "both", save_as),
#        plot = ggb2,
...
#   theme(legend.position = "right")
# 
# ggsave(sprintf("%s/plots/%s_random/line.%s", script.dir, Features, save_as),
#        plot = gg.line,
